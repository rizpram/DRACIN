...
export async function searchCatalogs(query:string,provider?:string):Promise<Drama[]>{
  ...
  const cacheKey=`search:${provider||"all"}:${q.toLowerCase()}`;
  return withSharedCache(cacheKey,TTL_SECONDS,async()=>{
    ...
    const defaultRows=await getDramas();
    const settled=await Promise.allSettled(CURATED.map(slug=>getCatalog(slug)));
    const rows=[...defaultRows,...settled.flatMap(x=>x.status==="fulfilled"?x.value:[])];
